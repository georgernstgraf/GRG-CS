...
public class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            var descriptor = services.First(d => d.ServiceType == typeof(DbContextOptions<OnlineStoreContext>));
            services.Remove(descriptor);
            services.AddDbContext<OnlineStoreContext>(options =>
            {
                options.UseSqlite("DataSource=onlinestore_test.db");
            });
        });
        builder.UseEnvironment("Testing");
    }
    public void InitializeDatabase(Action<OnlineStoreContext> action)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<OnlineStoreContext>();
    ...
    public Tout QueryDatabase<Tout>(Func<OnlineStoreContext, Tout> query)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<OnlineStoreContext>();
        return query(db);
